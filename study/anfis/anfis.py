...
class ANFIS:
    ...
    def plotResults(self):
        if self.isTrained:
            ...
            plt.plot(
                list(map(plusOne, list(range(len(self.fittedValues))))),
                self.fittedValues,
                'or',
                label='trained',
            )
            plt.plot(
                list(map(plusOne, list(range(len(self.testY))))),
                self.testY,
                '^b',
                label='original',
            )
            plt.hlines(
                [0.5], 0, len(self.fittedValues) + 1, "black", linestyles='dashed'
            )
    ...
    def aggregate(self):
        ...
        self.precision = (
            float(self.TP) / (self.TP + self.FP) if self.TP + self.FP != 0 else 0
        )
        self.recall = (
            float(self.TP) / (self.TP + self.TN) if self.TP + self.TN != 0 else 0
        )
        self.f_measure = (
...
def forwardHalfPass(anfis, trainData):
    ...
    for i in range(len(trainData[:, 0])):
        ...
        miAlloc = [
            [layerOne[x][anfis.rules[row][x]] for x in range(len(anfis.rules[0]))]
            for row in range(len(anfis.rules))
        ]
        ...
        rowHolder = np.concatenate(
            [x * np.append(trainData[i, :], 1) for x in layerThree]
        )
            ...
            for rowX in range(len(ANFISObj.trainX)):
                ...
                for colY in range(ANFISObj.trainY.ndim):
                    ...
                    for consequent in range(len(ANFISObj.rules[:, 0])):
                        fConsequent = np.dot(
                            np.append(ANFISObj.trainX[rowX, :], 1.0),
                            ANFISObj.consequents[
                                ((ANFISObj.trainX.shape[1] + 1) * consequent) : (
                                    ((ANFISObj.trainX.shape[1] + 1) * consequent)
                                    + (ANFISObj.trainX.shape[1] + 1)
                                ),
                                colY,
                            ],
                        )
                    ...
                    if ANFISObj.trainY.ndim == 1:
                        bucket2[colY] = (
                            sum1
                            * (ANFISObj.trainY[rowX] - theLayerFive[rowX, colY])
                            * (-2)
                        )
                    else:
                        bucket2[colY] = (
                            sum1
                            * (ANFISObj.trainY[rowX, colY] - theLayerFive[rowX, colY])
                            * (-2)
                        )
